  ...
  return (
     <div className = "home">
     {
       data.map((item)=>{
         ...
         <h5 style={{padding: "5px"}}><img style={{width: "4%",borderRadius: "100%"}}src={item.postedBy.pic} /><Link to={item.postedBy._id == state._id ?"/profile":`/profile/${item.postedBy._id}`}>{item.postedBy.name}</Link>{item.postedBy._id == state._id && <i onClick={()=>{deletePost(item._id)}} style={{float: "right"}} className="material-icons">delete</i>}</h5>
